...
from tools.consume.unified_receive import (find_stream, unified_receive,
                                           format_csv)
...
class TestUnifiedFindStream(unittest.TestCase):
    ...
    def setUp(self):
        ...
        """
        self.resolve_streams_patcher = patch('tools.consume.unified_receive.resolve_streams')
        self.stream_inlet_patcher = patch('tools.consume.unified_receive.StreamInlet')
        ...
        mock_stream1_desc.child.return_value.child.return_value = mock_stream1_ch1
        ...
        mock_stream2_desc.child.return_value.child.return_value = mock_stream2_ch1
        ...
        self.mock_resolve_streams.return_value = [self.mock_stream1, self.mock_stream2]
        ...
        self.mock_stream_inlet.side_effect = [self.mock_inlet1, self.mock_inlet2]
    ...
    def test_find_stream_not_found(self):
        ...
        self.assertIn("No specified LSL streams were found", str(context.exception))
...
class TestUnifiedReceiveData(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.open_patcher = patch('tools.consume.unified_receive.open', new_callable=mock_open)
        self.csv_writer_patcher = patch('tools.consume.unified_receive.csv.writer')

        self.mock_time = self.time_patcher.start()
        self.mock_open_func = self.open_patcher.start()
        self.mock_csv_writer = self.csv_writer_patcher.start()
        self.mock_time.time.side_effect = [0, 1, 2, 6]  # Start, loop, loop, end
    ...
    def test_unified_receive_creates_temp_file(self):
        ...
        self.mock_open_func.assert_called_once_with(temp_filename, "w", newline="")
        ...
        mock_writer_instance.writerow.assert_any_call([12345.1, "DSI-Stream", 1.0, 2.0])
        mock_writer_instance.writerow.assert_any_call([12345.2, "Marker-Stream", 100])
...
class TestFormatCsv(unittest.TestCase):
    ...
    def setUp(self):
        ...
        Set up the mocking environment for each test.
        """
        self.os_remove_patcher = patch('tools.consume.unified_receive.os.remove')
        self.open_patcher = patch('builtins.open', mock_open())
        self.read_csv_patcher = patch('tools.consume.unified_receive.pd.read_csv')
    ...
    def test_format_csv_success(self):
        ...
        written_data = "".join(call.args[0] for call in self.mock_open().write.call_args_list)
        self.read_csv_patcher.stop()
